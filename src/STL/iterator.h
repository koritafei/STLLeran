...
#ifndef __ITERTOR_H__
#define __ITERTOR_H__
...
struct iterator {
  ...
  typedef Pointer   pointer;            // 迭代器->操作结果的类型
  typedef Reference reference;          // 迭代器解引用操作结果类型
  typedef Distance difference_type;  // 迭代器距离类型(上述定义的五种迭代器类型)
};
...
template <class Iterator, bool>
struct iterator_traits_helpers {};

template <class Iterator>
struct iterator_traits_helpers<Iterator, true>
    : public iterator_traits_helpers<
          Iterator,
          std::is_convertible<typename Iterator::iterator_categpry,
                              input_iterator_tag>::value ||
              std::is_convertible<typename Iterator::iterator_category,
                                  output_iterator_tag>::value> {};

/**
 * @brief 萃取迭代器特性, 用于获取迭代器的相关类型
 * @tparam Iterator
 * */
template <class Iterator>
struct iterator_traits
    : public iterator_traits_helpers<Iterator,
...
void advance_distance(InputIterator& i, Distance n, input_iterator_tag) {
  while (--n) {
    ++i;
  }
}
...
void advance_distance(BidirectionalIter& i,
                      Distance           n,
...
// advance 的bidirectional_iterator_tag 的版本
template <class RandomIter, class Distance>
void advance_distance(RandomIter& i, Distance n, random_access_iterator_tag) {
  i += n;
}

template <class InputIterator, class Distance>
void advance(InputIterator& i, Distance n) {
  advance_distance(i, n, iterator_category(i));
}
